...
def GetArtist(ax,data):
    ...
    boids_scatter = ax.scatter([], [], s=BOID_SIZE)
    pred_scatter  = ax.scatter([], [], s=PRED_SIZE)

    boids0 = positions[0]
    vel0 = velocities[0]

    boids_quiver = ax.quiver(boids0[:, 0], boids0[:, 1],vel0[:, 0], vel0[:, 1], angles='xy', scale_units='xy', scale=ARROW_SCALE, width=ARROW_WIDTH,color="#ffffff" if DARK_MODE else "#000000")
