...
Simulations can be analysed using `ObservationLayer` which defines classes for getting readouts from a reservoir. Included is `FlatReadout` and `KernelReadout` which are different methods for reading the reservoir described in [Lymburn et al 2021](https://research-repository.uwa.edu.au/en/publications/reservoir-computing-with-swarms/). Additionally, the a superclass `ObservationAndPrediction` is included from which the two aforementioned readout classes extend. This exists for users who may want to define their own method of reading reservoir states.
